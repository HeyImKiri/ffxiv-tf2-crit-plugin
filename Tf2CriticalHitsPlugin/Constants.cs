...
private static ImmutableDictionary<uint, ISet<string>> InitActionPerJob()
{
    ...
    {
        var parentId = classJob.ClassJobParent.RowId;

        if (parentId != classJob.RowId && result.TryGetValue(parentId, out var parentActions))
        {
    ...
    foreach (var (key, value) in result.ToList())
    {
